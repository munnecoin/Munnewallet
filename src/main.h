...
static const int64_t MAX_MINT_PROOF_OF_STAKE = 0.03 * COIN;	// 3% yearly interest
...
public:
    ...
    {
        // Munne: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const;  // Munne: get transaction coin age
...
public:
    ...
    // Munne: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // Munne: two types of block: proof-of-work or proof-of-stake
    ...
    // Munne: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64_t& nCoinAge) const; // Munne: calculate total coin age spent in block
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // Munne: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // Munne: block index flags
    enum  
    {
