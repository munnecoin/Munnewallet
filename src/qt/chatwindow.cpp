...
QString urlReddit = "http://www.reddit.com/r/munne/.rss";
...
void ChatWindow::getRequest(const QString &urlString )
{
    QUrl url (urlString);
...
void ChatWindow::processNetworkResponse(QNetworkReply *finished )
{
...
void ChatWindow::fetch()
{



    xml.clear();
    getRequest(urlReddit);



}

void ChatWindow::metaDataChanged()
{
...
void ChatWindow::finished(QNetworkReply *reply)
{
    Q_UNUSED(reply);
    // UI Changes
}

void ChatWindow::parseXml(QXmlStreamReader &myxml)
{
        ...
        if (myxml.isStartElement())
        {
         ...
         } else if (myxml.isEndElement())
         {
            if (myxml.name() == "item")
            {
                ...
                // Remove first title
                titleString.remove("Munne: A new economy is here");
                // UI Changes

                txt = txt + "<p><a href='" + linkString + "' style='color:#111;text-transform:uppercase;font-weight:800;text-decoration:none;'>" + titleString + "</a></p> <p><span style='color:#999;'>" + date.mid(0, 17) +"</span></p>";
         ...
         } else if (myxml.isCharacters() && !myxml.isWhitespace())
         {
             ...
             {
                 date += myxml.text().toString();
             }

         }

         ui->newsLabel->setText(txt);
...
void ChatWindow::error(QNetworkReply::NetworkError)
{
