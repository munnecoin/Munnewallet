...
static const int64_t nTargetTimespan_legacy = nTargetSpacing * nRetarget; // every 100 blocks
static const int64_t nInterval = nTargetTimespan_legacy / nTargetSpacing;

static const int64_t nTargetTimespan = 100 * 90;
...
const string strMessageMagic = "Munne Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // Munne: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
unsigned int CTransaction::GetLegacySigOpCount(const CTransaction& tx)
{
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Munne: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// Munne: find last block index up to pindex
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Includes Munne fix for wrong retargeting difficulty by Mammix2
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Munne: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Munne: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // Munne: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // Munne: track money supply and mint amount info
...
// Munne: total coin age spent in transaction, in the unit of coin-days.
...
// Munne: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // Munne: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // Munne: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // Munne: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // Munne: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // Munne: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Munne: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // Munne: getblocks may not obtain the ancestor block rejected
    ...
    // Munne: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Munne", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "29/6";
        CTransaction txNew;
        txNew.nTime = 1404044346;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1404044346;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1265346;
        if(fTestNet)
        {
            block.nNonce   = 1265346;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x3a399f4acc0f247e48d187104e91000c114c2289b089bf79a5935b413e583073"));
        ...
        // Munne: initialize synchronized checkpoint
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // Munne: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // Munne: tell downloading node about the latest block if it's
